...
public class DeckView {
    ...
    public static void renderPlayerDeck(GraphicsContext gc, Image icon1, Image icon2, Image icon3, Image icon4, double globalScale) {
        gc.setGlobalAlpha(0.5);
        gc.fillRoundRect(0, gc.getCanvas().getHeight() - 150, gc.getCanvas().getWidth(), 150, 0, 0);
        gc.setGlobalAlpha(1);

        double xCenter = gc.getCanvas().getWidth()/2;
        double yCenter = gc.getCanvas().getHeight()* 0.90;
        ...
        double fixedWidth = 100 * globalScale;
        double fixedHeight = 100 / OUTLINE_DEFAULT_SPELL.getWidth() * OUTLINE_DEFAULT_SPELL.getHeight() * globalScale;

        gc.drawImage(icon2, xCenter - fixedWidth/2, yCenter - fixedHeight/2, fixedWidth, fixedHeight);
